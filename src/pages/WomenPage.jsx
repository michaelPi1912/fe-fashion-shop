...
export default function WomenPage(){
  ...
  const fetchData = (count) =>{
    ...
    fetch(`${process.env.REACT_APP_API_URL}/api/v1/product/list?category=Women&size=${count}`).then(
        res => res.json()).then(
        json => {
            setProducts(json)
  ...
  const Filter = (count) =>{
    ...
    if(cate === "all"){
        ...
        if(min > max){
            ...
            console.log(params)
        fetch(`${process.env.REACT_APP_API_URL}/api/v1/product/list?category=Women&size=${count}&`+params).then(
          res => res.json()).then(
          json => {
            setProducts(json)
            setIsFilter(true)
